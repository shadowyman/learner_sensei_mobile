...
* For any ExecPlan that migrates or introduces an LLM tool, include an explicit “Mobile Routing Gate” milestone in `Progress` and `Plan of Work`. The gate must cover: wiring the mobile WebView build to a BFF-backed path for that task, gating desktop-only local SDK paths with `window.__SENSEI_MOBILE_BUILD__`, and adding a sentinel test that fails if mobile uses a browser `CoreLlmClient`.
