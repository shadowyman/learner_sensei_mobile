...
- [x] (2026-06-05 00:30Z) PR #1 review comments inspected through `gh api`, full analyzer snapshot refreshed, and remediation scope narrowed to the first five inline comments: module-intro reload routing, main-response history, base persona preservation, BFF input length cap, and LLM stream timeout alignment. The sixth comment about the live Gemini smoke in default `bff npm test` is deliberately excluded because the user explicitly confirmed that default inclusion is intentional.
- [x] (2026-06-05 00:30Z) Root-cause remediation ledger created at `tmp/llm_stream_pr_review_rCA_20260605T003038Z.md` and Core Analysis checkpoint created at `docs/mission_state/mission_state_llm_stream_pr_review_remediation_20260605T003038Z.md`.
- [x] (2026-06-05 00:43Z) Implemented robust fixes for the five accepted PR-review issues: Core/BFF prompt envelope restores base persona, WebView sends bounded structured conversation history for main streams, module-intro reload reuses the structured stream request, BFF enforces 413 on oversized direct learner inputs, and active LLM stream timeout uses the larger main-response provider timeout.
- [x] (2026-06-05 00:43Z) Added regression coverage for the five fixes. `__tests__/corePromptParity.test.ts` covers base persona/history envelopes; `__tests__/ModuleSelectionHandler.test.ts` covers module-intro reload context; `bff/tests/llmStream.deterministic.int.test.js` covers persona/history reaching the gateway, 413 guards, and timeout alignment.
- [ ] (2026-06-05 00:43Z) Run broad validation, commit, push, and reply to the five GitHub review comments with `@codex review`.
...
- Observation: PR #1 review exposed that the first migration pass moved transport and prompt ownership, but did not fully preserve browser-chat semantics for the mobile capability path.
  Evidence: current source shows `mainSenseiLlmStreamRequest` carries current-turn prompt ingredients only; `GeminiGateway.streamMainResponse` sends a single `contents` prompt without the browser chat's `SENSEI_SYSTEM_INSTRUCTION_BASE_PERSONA_AND_COMMITMENTS`; module-intro reload does not pass `llmStreamRequest`; `/llm-stream` lacks the `/turns` `MAX_INPUT_CHARS` guard; and LLM capability streams use the generic 60 second hard stream timeout while Gemini main response timeout is longer.

- Observation: The live Gemini smoke being in default `bff npm test` remains intentionally different from typical CI isolation advice.
  Evidence: the user explicitly directed that the smoke test should be in default BFF `npm test` like existing live BFF tests, and later confirmed the review comment asking to remove it from default is not to be fixed in this pass.

- Observation: The first focused validation pass exposed two harness issues rather than new product defects.
  Evidence: root Core prompt tests imported built `@sensei/core` output before `npm run core:build` refreshed `dist`, so the new envelope options were not visible to Jest. The BFF deterministic test also hit the existing 3-request per-minute stream rate limit after adding a fourth accepted stream; the timeout case must use a fresh session so it tests timeout behavior rather than rate limiting.
...
- Decision: Fix only the first five PR #1 review comments and leave the default live Gemini smoke unchanged.
  Rationale: The first five comments identify parity or guardrail regressions in the migrated capability path. The sixth comment is operationally reasonable in generic CI terms, but conflicts with the user's deliberate requirement that the real Gemini smoke be part of default BFF `npm test` for this migration.
  Date/Author: 2026-06-05 / Coding agent at user direction.

- Decision: Preserve main-response history by sending bounded structured conversation history from WebView and letting Core assemble the final prompt server-side, rather than sending final prompt text from mobile.
  Rationale: The master plan forbids mobile from owning final prompt text for migrated capabilities. Structured role/text history preserves browser-chat semantics while keeping final prompt assembly in Core/BFF.
  Date/Author: 2026-06-05 / Coding agent.

- Decision: Preserve the base Recursive Sensei persona inside the Core/BFF prompt envelope for migrated streams.
  Rationale: The browser path used Gemini chat `systemInstruction`, but the BFF streaming gateway currently calls `generateContentStream` with a single prompt in `contents`. Including the base persona in the server-built prompt restores the teaching contract without moving provider execution or final prompt assembly back to mobile.
  Date/Author: 2026-06-05 / Coding agent.
...
- PR review remediation for the next review batch completed on 2026-06-05 for the first five actionable comments only. Fixed scope: module-introduction reloads now preserve structured native/BFF routing data; `/llm-stream` now enforces the existing direct learner-input length cap and preserves validated structured history; migrated main-response streams now send bounded structured conversation history; BFF/Core now add a Core-owned Recursive Sensei base instruction envelope server-side for migrated capability prompts; and active LLM stream timeout now honors the larger main-response provider timeout instead of the generic pending-request timeout. The live-Gemini default-suite smoke comment was intentionally left unchanged because the user confirmed that existing default BFF behavior is deliberate.
- PR review remediation validation for this batch passed before commit. Commands: `node --check bff/src/controllers/sessionController.js && node --check bff/src/services/streamingService.js && node --check bff/src/integration/senseiCoreAdapter.js`; `npm run core:build`; `npm test -- --runTestsByPath __tests__/corePromptParity.test.ts __tests__/ModuleSelectionHandler.test.ts __tests__/interactionHelpers.test.ts --silent --bail --noStackTrace`; `cd bff && node tests/llmStream.deterministic.int.test.js`; `npm run webview:bundle`; `npm test -- --silent --bail --noStackTrace`; `cd SenseiMobile && npm test -- --runInBand`; `npm run analysis:run -- --include src/index.tsx,src/moduleSelectionHandler,src/interactionHelpers,src/ui.ts,core/mainSenseiResponse,core/moduleIntroduction,core/promptEnvelope,bff/src/controllers/sessionController,bff/src/services/streamingService,bff/src/integration/senseiCoreAdapter`; and `git diff --check`. Results: syntax checks passed; Core build passed; focused root tests passed 3 suites/24 tests with 1 todo; deterministic BFF stream integration passed; WebView bundle passed; full root Jest passed 37 suites/273 tests with 4 todo; mobile Jest passed 2 suites/5 tests; scoped analyzer completed; whitespace check passed.
- PR branch update completed after this remediation batch. Commit `790d9e7` was pushed to `origin/codex/llm-streaming-core-bff-migration`, and replies were posted to GitHub review comments `3359567378`, `3359567383`, `3359567385`, `3359567392`, and `3359567396`, each ending with `@codex review` as requested. Comment `3359567390` was not changed in this batch because the user confirmed that default BFF live-Gemini smoke behavior was a deliberate decision.
