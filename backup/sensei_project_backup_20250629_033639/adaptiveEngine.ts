...
export function updateLearnerModel(
    ...
    if (analysis) {
        ...
        if (curriculumState?.currentPhase === 'Socratic') {
            logger.info('[LEARNER_DECOUPLE] Updated affective state for Socratic phase:', {
                confidence: model.AffectiveState.Confidence,
                engagement: model.AffectiveState.Engagement,
                confusion: model.AffectiveState.Confusion
            });
        }
        ...
        if (curriculumState && analysis.key_content_point_assessment && expectedContentPointTextsForCurrentChunk && curriculumState.currentPhase !== 'Socratic') {
            logger.info('[LEARNER_DECOUPLE] Processing content point assessments, expected points:', expectedContentPointTextsForCurrentChunk.length);
            ...
            logger.info('[LEARNER_DECOUPLE] Skipping assessment processing for Socratic phase');
        }
